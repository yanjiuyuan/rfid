...
namespace DingTalk.Controllers
{
    ...
    public class BorrowController : ApiController
    {
        ...
        [Route("Save")]
        [HttpPost]
        public NewErrorModel Save([FromBody] List<Borrow> goDownList)
        {
            try
            {
                EFHelper<Borrow> eFHelper = new EFHelper<Borrow>();
        ...
        public object Read(string taskId)
        {
            ...
            {
                EFHelper<Borrow> eFHelper = new EFHelper<Borrow>();
                List<Borrow> goDownList = eFHelper.GetListBy(t => t.TaskId == taskId).ToList();
                return new NewErrorModel()
                {
        ...
        public async Task<object> PrintAndSend([FromBody]PrintModel printAndSendModel)
        {
            try
            {
                ...
                using (DDContext context = new DDContext())
                {
                    ...
                    List<Borrow> GoDownList = context.Borrow.Where(u => u.TaskId == TaskId).ToList();
        ...
        public async Task<object> PrintExcel([FromBody]PrintModel printAndSendModel)
        {
            try
            {
                using (DDContext context = new DDContext())
                {
                    List<Borrow> purchaseTables = context.Borrow.Where(p => p.TaskId == printAndSendModel.TaskId).ToList();
                    DataTable dtpurchaseTables = ClassChangeHelper.ToDataTable(purchaseTables);

                    string path = HttpContext.Current.Server.MapPath("~/UploadFile/Excel/Templet/借入导出模板.xlsx");
                    string time = DateTime.Now.ToString("yyyyMMddHHmmss");
                    string newPath = HttpContext.Current.Server.MapPath("~/UploadFile/Excel/Templet") + "\\借入单" + time + ".xlsx";
                    File.Copy(path, newPath);
                    if (ExcelHelperByNPOI.UpdateExcel(newPath, "Sheet1", dtpurchaseTables, 0, 1))
                    {
                        DingTalkServersController dingTalkServersController = new DingTalkServersController();
                        //上盯盘
                        var resultUploadMedia = await dingTalkServersController.UploadMedia("~/UploadFile/Excel/Templet/借入单" + time + ".xlsx");
                        //推送用户
                        FileSendModel fileSendModel = JsonConvert.DeserializeObject<FileSendModel>(resultUploadMedia);
